...
class Detector(QThread):
    ...
    det_log_out_sign = pyqtSignal(int, str)
    test_det_res_sign = pyqtSignal(np.ndarray)
    test_det_ok = pyqtSignal()

    def __init__(self):
        super().__init__()
        self.test_det_status = False
        ...
        self.test_det_sign.connect(self.test_det_slot)

    def run(self) -> None:
        pass

    def test_det_work(self):
        cap = cv2.VideoCapture("../datas/test_videos/test_video.mp4")
        if not cap.isOpened():
            self.det_log_out_sign.emit(2, "测试视频打开错误,请检查文件夹中是否存在用于测试的视频文件!")
            return
        while True:
            if self.test_det_status:
                return
            flag, frame = cap.read()
            if not flag:
                self.det_log_out_sign.emit(0, "测试视频播放完成!")
                self.test_det_ok.emit()
                return
            frame = self.getRoiRect(frame)
            s1 = time.time()
            _, res_draw = self.detect2(frame, 1, 0.6)
            s2 = time.time() - s1
            res_draw = cv2.cvtColor(res_draw, cv2.COLOR_BGRA2BGR)
            self.det_log_out_sign.emit(0, "*-调试检测-* 检测一帧耗时 {} ms".format(s2 * 1000))
            self.test_det_res_sign.emit(res_draw)

    def test_det_slot(self, flag):
        if flag:
            t = Thread(target=self.test_det_work)
            t.setDaemon(True)
            t.start()
            self.test_det_status = False
        else:
            self.test_det_status = True
